...
describe('Agent Memory CRUD Integration Tests', () => {
  ...
  const AGENT_WITH_MEMORIES: AgentProfile = {
    ...
    memories: [testMemory1, testMemory2],
    created_at: new Date('2024-01-01T00:00:00Z'),
  ...
  describe('記憶 CRUD 完整流程測試', () => {
    ...
    test('應該能夠讀取帶有記憶的 Agent', async () => {
      ...
      expect(readData.data.memories).toHaveLength(2);
      expect(readData.data.memories).toEqual(
        expect.arrayContaining([
          expect.objectContaining({ content: testMemory1.content, type: testMemory1.type }),
          expect.objectContaining({ content: testMemory2.content, type: testMemory2.type })
        ])
      );
    });
    ...
    test('應該能夠更新 Agent 的記憶', async () => {
      ...
      const updateInput = {
        memories: [testMemory1, testMemory2, newMemory]
      };
      ...
      expect(updateData.data.memories).toHaveLength(3);
      expect(updateData.data.memories).toEqual(
        expect.arrayContaining([
          expect.objectContaining({ content: newMemory.content, type: newMemory.type })
        ])
      );
    });
    ...
    test('應該能夠刪除 Agent 中的特定記憶', async () => {
      ...
      expect(updateResponse.status).toBe(200);
      expect(updateData.success).toBe(true);
      expect(updateData.data.memories).toHaveLength(1);
      expect(updateData.data.memories[0]).toEqual(
        expect.objectContaining({ content: testMemory2.content, type: testMemory2.type })
      );
    });
    ...
    test('應該能夠處理記憶標籤', async () => {
      ...
      expect(updateResponse.status).toBe(200);
      expect(updateData.success).toBe(true);
      expect(updateData.data.memories[0].tags).toEqual(['重要', '客戶', '產品']);
    });
  });
  ...
  describe('記憶邊界情況測試', () => {
    test('應該能夠處理空的記憶列表', async () => {
      ...
      expect(updateResponse.status).toBe(200);
      expect(updateData.success).toBe(true);
      expect(updateData.data.memories).toHaveLength(0);
    });
    ...
    test('應該能夠處理大量記憶', async () => {
      ...
      expect(updateResponse.status).toBe(200);
      expect(updateData.success).toBe(true);
      expect(updateData.data.memories).toHaveLength(10);
    });
  });
}); 